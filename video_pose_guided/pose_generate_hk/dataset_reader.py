...
class DataLoader:
    ...
    def __init__(self):
        ...
        #self.extract()
        # Generate pairs 
        # I comment this while testing my own images
        '''
        ...
        '''
    def process_oneimg(self, fulldir,kp_path = None,only_img = False, index = 0):

        if not "flat" in fulldir:
            img = cv2.imread(fulldir)  #(750,1101,3)
            print(fulldir)
            print(img.shape)
            ...
            if img is not None:
                ...
                #keypointfileDir = fulldir[:-3] + "pkl"   #for image
                keypointfileDir = kp_path +"keypoints.pkl"
                  
                with open(keypointfileDir, 'rb') as kpfile:
                    allkeypoints = pickle.load(kpfile)
                    availablePoints = []
                    keypoints = allkeypoints[index]  #the index in allkeypoints will be used
                ...
                #cv2.imwrite("keypoints/lines.jpg",mapofAllPoints)
                ...
                #print(heatmap.shape)
                #cv2.imwrite("keypoints/dilatedMapofAllPoints.jpg",dilatedMapofAllPoints)
                #sys.exit()
    ...
    def next_batch(self, batch_size, trainorval,targetimg_path):
        ...
        for i in range(batch_size):
            ...
            condimg_dir = "/data/shaoswan/phd/motion/pose_generate_hk/dataset/test/04_4_full.jpg"
            #print("dataset_reader/condimg_dir",condimg_dir)
      
            
            conditional_image[i], _,_ = self.process_oneimg(condimg_dir,only_img = True)
            
            #targetimg_dir = self.index2dir[pairstofeed[i][1]]
            #  "/data/shaoswan/phd/motion/pose_generate_hk/dataset/kick004/"
            targetimg_dir = targetimg_path + str(i+1) +".jpg"
            
            #kp_path = "/data/shaoswan/phd/motion/pose_generate_hk/dataset/kick004/videokeypoints.pkl"
            
            # targetimg_path pickle and images dir 
            # targetimg_dir image
            print("targetimg_dir",targetimg_dir)
            print("Done",i)
            target_image[i], target_pose[i], target_morphologicals[i] = self.process_oneimg(targetimg_dir,kp_path  = targetimg_path,index = i)
            #sys.exit()
